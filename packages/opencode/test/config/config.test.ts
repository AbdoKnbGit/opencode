      ...
      Effect.sync(() => {
        ...
        if (typeof payload === "object" && payload !== null && "config" in payload) {
          ...
          if (typeof value === "object" && value !== null) {
            docs.push({ url, source, dir: path.dirname(source), content: value })
          }
        }
        if (typeof payload === "object" && payload !== null && "remote_config" in payload) {
          const remoteConfig = payload.remote_config
          if (typeof remoteConfig === "object" && remoteConfig !== null && "url" in remoteConfig) {
            const remoteUrl =
              typeof remoteConfig.url === "string"
                ? remoteConfig.url.replaceAll("{env:TEST_TOKEN}", "test-token")
                : undefined
            if (remoteUrl) {
              seen.remote = remoteUrl
              const headers = "headers" in remoteConfig ? remoteConfig.headers : undefined
              if (typeof headers === "object" && headers !== null && "Authorization" in headers) {
                const authorization = headers.Authorization
                if (typeof authorization === "string") {
                  seen.authorization = authorization.replaceAll("{env:TEST_TOKEN}", "test-token")
                }
              }
              const remote = input.remote
              const content =
                typeof remote === "object" && remote !== null && "config" in remote
                  ? remote.config
                  : remote
              docs.push({
                url: remoteUrl,
